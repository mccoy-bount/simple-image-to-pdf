...
const calcImageLayout = (
  [pageWidth, pageHeight]: ImageSize,
...
const genPDFPage = async ({
  ...
  const imagePDF = await doc.embedJpg(await sharper.toBuffer());
  ...
  if (settings.autoSize) {
    ...
    const {
      ...
      width: imageWidth,
      height: imageHeight,
    } = calcImageLayout(settings.size, [width, height]);
    page.drawImage(imagePDF, {
      x,
      y,
      width: imageWidth,
